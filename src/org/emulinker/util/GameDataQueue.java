...
public class GameDataQueue {
    ...
    public GameDataQueue(int gameID, int numPlayers, int timeoutMillis,
            int retries) {
        this.gameID = gameID;
    ...
    public byte[] getData(int playerNumber, int byteCount, int bytesPerAction)
        ...
        return playerQueues[(playerNumber - 1)].getData(byteCount,
                bytesPerAction);
    }
    ...
    private class PlayerDataQueue {
        ...
        private PlayerDataQueue(int playerNumber) {
            ...
            for (int i = 0; i < queues.length; i++) {
                queues[i] = new CircularBlockingByteQueue(
                        ((numPlayers * 6) * 4));
            }
        }
        ...
        private byte[] getData(int byteCount, int bytesPerAction)
            ...
            if (lastData != null) {
                ...
                // log.debug("Player " + thisPlayerNumber + ": getData with i="
                // + lastI + ", j=" + lastJ);
            }
            else {
            ...
            for (int i = lastI; i < ((byteCount / bytesPerAction) * numPlayers); i++) {
                for (int j = lastJ; j < bytesPerAction; j++) {
                    try {
                        data[((i * bytesPerAction) + j)] = queues[(i % numPlayers)]
                        ...
                        if (++timeoutCounter > retries) {
                            throw new DesynchException("Player "
                                    + ((i % numPlayers) + 1) + " is lagged!",
                                    ((i % numPlayers) + 1), e);
                        }
                        else {
                            throw new PlayerTimeoutException(
                                    ((i % numPlayers) + 1), timeoutCounter, e);
                        }
                    }
                }
            }
    ...
    public static class PlayerTimeoutException extends Exception {
        ...
        private PlayerTimeoutException(int playerNumber, int timeoutNumber,
                TimeoutException e) {
            super(e);
    ...
    public static class DesynchException extends Exception {
        ...
        private DesynchException(String msg, int playerNumber,
                TimeoutException e) {
            super(msg, e);
            this.playerNumber = playerNumber;
        }
