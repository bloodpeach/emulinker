...
public abstract class UDPRelay2 {
    ...
    public synchronized void start() throws IOException {
        if (!started) {
            stopFlag = false;
            relayThreads.put(serverSocketAddress, new RelayThread(listenPort,
                    serverSocketAddress));
            started = true;
        }
        else {
    ...
    protected class RelayThread extends Thread {
        ...
        protected RelayThread(InetSocketAddress forwardAddress)
                throws IOException {
            this(-1, forwardAddress);
        }

        protected RelayThread(int port, InetSocketAddress forwardAddress)
                throws IOException {
            if (port > 0) {
            ...
            this.name = "RelayThread." + threadCounter++ + ": "
                    + channel.socket().getLocalPort() + "->"
                    + EmuUtil.formatSocketAddress(forwardAddress);
            this.start();
        ...
        public void send(ByteBuffer buffer, InetSocketAddress target)
                throws IOException {
            ...
            try {
                ...
                while (!stopFlag) {
                    ...
                    if (fromAddress.equals(getServerSocketAddress())) {
                        ...
                        sendBuffer = processServerToClient(receiveBuffer,
                                fromAddress, getForwardAddress());
                    }
                    else {
                        // log.debug("Client at " +
                        // EmuUtil.formatSocketAddress(fromAddress) + " sent " +
                        ...
                        // log.debug("Buffer Dump: " +
                        // EmuUtil.dumpBuffer(receiveBuffer));
                        sendBuffer = processClientToServer(receiveBuffer,
                                fromAddress, getForwardAddress());
                    }
                    ...
                    RelayThread responseThread = (RelayThread) relayThreads
                            .get(fromAddress);
