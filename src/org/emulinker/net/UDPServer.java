...
public abstract class UDPServer implements Executable, Startable {
    ...
    protected void send(ByteBuffer buffer, InetSocketAddress toSocketAddress) {
         ...
         * Math.abs(random.nextInt()%100) < artificalPacketLossPercentage) {
         * return; }
         */
        try {
            ...
            log.error(
                    "Failed to send on port " + getBindPort() + ": "
                            + e.getMessage(), e);
        }
    }
        ...
        try {
            while (!stopFlag) {
                try {
                     ...
                     * artificalPacketLossPercentage) { releaseBuffer(buffer);
                     * continue; }
                     * 
                     * if(artificalDelay > 0) { try {
                     * Thread.sleep(artificalDelay); } catch(Exception e) {} }
                     */
                    buffer.flip();

                    // log.debug("receive("+EmuUtil.dumpBuffer(buffer,
                    // false)+")");
                    ...
                    log.error("Failed to receive on port " + getBindPort()
                            + ": " + e.getMessage());
                }
                catch (IOException e) {
                    if (stopFlag) {
            ...
            log.fatal("UDPServer on port " + getBindPort()
