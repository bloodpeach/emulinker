...
public class KailleraServerImpl implements KailleraServer, Executable {
    ...
    public KailleraServerImpl(ThreadPoolExecutor threadPool,
        ...
        allowMultipleConnections = config
                .getBoolean("server.allowMultipleConnections");
        ...
        for (int i = 1; i <= 999; i++) {
            if (EmuLang.hasString("KailleraServerImpl.LoginMessage." + i)) {
                loginMessages.add(EmuLang
                        .getString("KailleraServerImpl.LoginMessage." + i));
            }
            else {
                break;
            }
        ...
        if (gameTimeoutMillis <= 0) {
            throw new ConfigurationException(
                    "game.timeoutMillis can not be <= 0");
        }

        gameDesynchTimeouts = config.getInt("game.desynchTimeouts");

        gameAutoFireSensitivity = config
                .getInt("game.defaultAutoFireSensitivity");
        ...
        List<String> connectionTypes = config
                .getList("server.allowedConnectionTypes");

        for (String s : connectionTypes) {
            try {
                int ct = Integer.parseInt(s);
                if (ct < 1 || ct > 6) {
                    throw new ConfigurationException("Invalid connectionType: "
                            + s);
                }
        ...
        boolean touchKaillera = config.getBoolean("masterList.touchKaillera",
                false);
    ...
    public synchronized void start() {
        ...
        log.debug("KailleraServer thread starting (ThreadPool:"
    ...
    protected AutoFireDetector getAutoFireDetector(KailleraGame game) {
        ...
        return autoFireDetectorFactory.getInstance(game,
                gameAutoFireSensitivity);
    }
    ...
    public synchronized KailleraUser newConnection(
            InetSocketAddress clientSocketAddress, String protocol,
            KailleraEventListener listener) throws ServerFullException,
            NewConnectionException {
        ...
        KailleraUserImpl user = new KailleraUserImpl(userID, protocol,
                clientSocketAddress, listener, this);
        user.setStatus(KailleraUser.STATUS_CONNECTING);

        log.info(user + " attempting new connection using protocol " + protocol
                + " from " + EmuUtil.formatSocketAddress(clientSocketAddress));

        log.debug(user + " Thread starting (ThreadPool:"
                + threadPool.getActiveCount() + "/" + threadPool.getPoolSize()
                + ")");
        threadPool.execute(user);
        Thread.yield();
        log.debug(user + " Thread started (ThreadPool:"
                + threadPool.getActiveCount() + "/" + threadPool.getPoolSize()
                + ")");
    ...
    public synchronized void login(KailleraUser user) throws PingTimeException,
        ...
        log.info(user
                + ": login request: delay=" + loginDelay + "ms, clientAddress=" + EmuUtil.formatSocketAddress(user.getSocketAddress()) + ", name=" + user.getName() + ", ping=" + user.getPing() + ", client=" + user.getClientType() + ", connection=" + KailleraUser.CONNECTION_TYPE_NAMES[user.getConnectionType()]); //$NON-NLS-4$ //$NON-NLS-5$
        ...
        int access = accessManager.getAccess(user.getSocketAddress()
                .getAddress());
        ...
        if (access == AccessManager.ACCESS_NORMAL && getMaxPing() > 0
            ...
            log.info(user + " login denied: Ping " + user.getPing() + " > "
                    + getMaxPing());
            users.remove(userListKey);
            throw new PingTimeException(EmuLang.getString(
        ...
        if (access == AccessManager.ACCESS_NORMAL
            ...
            log.info(user
                    ...
                    + KailleraUser.CONNECTION_TYPE_NAMES[user
                            .getConnectionType()] + " Not Allowed");
            users.remove(userListKey);
            throw new LoginException(
                    EmuLang.getString(
        ...
        if (user.getPing() < 0) {
            ...
            users.remove(userListKey);
            throw new PingTimeException(EmuLang.getString(
                    "KailleraServerImpl.LoginErrorInvalidPing", user.getPing()));
        }
        ...
        if (maxUserNameLength > 0
            ...
            log.info(user + " login denied: UserName Length > "
        ...
        if (access == AccessManager.ACCESS_NORMAL && maxClientNameLength > 0
            ...
            log.info(user + " login denied: Client Name Length > "
                    + getMaxClientNameLength());
            users.remove(userListKey);
            throw new UserNameException(
        ...
        if (access == AccessManager.ACCESS_NORMAL) {
            ...
            for (int i = 0; i < chars.length; i++) {
                if (chars[i] < 32) {
                    log.info(user
                            + " login denied: Illegal characters in UserName");
                    users.remove(userListKey);
                    throw new UserNameException(
        ...
        if (u.getStatus() != KailleraUser.STATUS_CONNECTING) {
            ...
            throw new LoginException(
                    EmuLang.getString(
                            "KailleraServerImpl.LoginErrorInvalidStatus",
                            u.getStatus()));
        }
        ...
        if (access == AccessManager.ACCESS_NORMAL
            ...
            log.info(user + " login denied: AccessManager denied emulator: "
                    + user.getClientType());
            users.remove(userListKey);
            throw new LoginException(EmuLang.getString(
                    "KailleraServerImpl.LoginDeniedEmulatorRestricted",
                    user.getClientType()));
        }
            ...
            if (u2.isLoggedIn()) {
                if (!u2.equals(u)
                        && u.getConnectSocketAddress()
                                .getAddress()
                                .equals(u2.getConnectSocketAddress()
                                        .getAddress())
                        && u.getName().equals(u2.getName())) {
                    // user is attempting to login more than once with the same
                    // name and address
                    ...
                    try {
                        ...
                        log.error("Error forcing " + u2
                                + " quit for reconnect!", e);
                    }
                }
                else if (!u2.equals(u)
                        && u2.getName().toLowerCase().trim()
                                .equals(u.getName().toLowerCase().trim())) {
                    users.remove(userListKey);
                    log.warn(user
                            + " login denied: Duplicating Names is not allowed! "
                            + u2.getName());
                    throw new ClientAddressException(
                            "Duplicating names is not allowed: " + u2.getName());
                }
                ...
                if (access == AccessManager.ACCESS_NORMAL
                    ...
                    users.remove(userListKey);
                    log.warn(user
                            + " login denied: Address already logged in as "
                            + u2.getName());
                    throw new ClientAddressException(EmuLang.getString(
                            "KailleraServerImpl.LoginDeniedAlreadyLoggedInAs",
                            u2.getName()));
                }
            }
        }
        ...
        if (access > AccessManager.ACCESS_NORMAL) {
            log.info(user + " logged in successfully with "
        ...
        if (user.isEmuLinkerClient()) {
            userImpl.addEvent(new InfoMessageEvent(user, ":ACCESS="
                    + userImpl.getAccessStr()));
            ...
            if (access >= AccessManager.ACCESS_SUPERADMIN) {
                ...
                for (KailleraUserImpl u3 : getUsers()) {
                    ...
                    sb.append(u3.getID());
                    sb.append((char) 0x02);
                    sb.append(u3.getConnectSocketAddress().getAddress()
                            .getHostAddress());
                    sb.append((char) 0x02);
                    ...
                    if (sb.length() > 300) {
                        ((KailleraUserImpl) user)
                                .addEvent(new InfoMessageEvent(user, sb
                                        .toString()));
                ...
                if (sbCount > 0) {
                    ((KailleraUserImpl) user).addEvent(new InfoMessageEvent(
                            user, sb.toString()));
                }
        ...
        if (access >= AccessManager.ACCESS_ADMIN) {
            userImpl.addEvent(new InfoMessageEvent(user, EmuLang
                    .getString("KailleraServerImpl.AdminWelcomeMessage")));
        }
        ...
        userImpl.addEvent(new InfoMessageEvent(user, getReleaseInfo()
                .getProductName()
                + " v"
                + getReleaseInfo().getVersionString()
                + ": "
                + getReleaseInfo().getReleaseDate()
                + " - Visit: http://www.insiXide.com/EmulinkerX"));
        ...
        String announcement = accessManager.getAnnouncement(user
    ...
    public synchronized void quit(KailleraUser user, String message)
            throws QuitException, DropGameException, QuitGameException,
            CloseGameException {
        if (!user.isLoggedIn()) {
            users.remove(user.getID());
            log.error(user + " quit failed: Not logged in");
            throw new QuitException(
                    EmuLang.getString("KailleraServerImpl.NotLoggedIn"));
        }
        ...
        if (quitMsg.length() == 0
            ...
            quitMsg = EmuLang
                    .getString("KailleraServerImpl.StandardQuitMessage");
        }

        int access = user.getServer().getAccessManager()
                .getAccess(user.getSocketAddress().getAddress());

        if (access < AccessManager.ACCESS_SUPERADMIN
                && user.getServer().getAccessManager()
                        .isSilenced(user.getSocketAddress().getAddress())) {
            quitMsg = "http://www.insiXide.com/EmulinkerX";
        }
    ...
    public synchronized void chat(KailleraUser user, String message)
        ...
        if (access < AccessManager.ACCESS_SUPERADMIN
                && accessManager.isSilenced(user.getSocketAddress()
                        .getAddress())) {
            log.warn(user + " chat denied: Silenced: " + message);
            throw new ChatException(
                    EmuLang.getString("KailleraServerImpl.ChatDeniedSilenced"));
        }

        if (access == AccessManager.ACCESS_NORMAL
                && chatFloodTime > 0
                && (System.currentTimeMillis() - ((KailleraUserImpl) user)
                        .getLastChatTime()) < (chatFloodTime * 1000)) {
        ...
        if (access == AccessManager.ACCESS_NORMAL) {
            ...
            for (int i = 0; i < chars.length; i++) {
                if (chars[i] < 32) {
                    log.warn(user
                            + " chat denied: Illegal characters in message");
                    throw new ChatException(
            ...
            if (maxChatLength > 0 && message.length() > maxChatLength) {
                log.warn(user + " chat denied: Message Length > "
                        + maxChatLength);
                throw new ChatException(
        ...
        if (switchTrivia) {
            if (!trivia.isAnswered() && trivia.isCorrect(message)) {
                trivia.addScore(user.getName(), user.getSocketAddress()
                        .getAddress().getHostAddress(), message);
            }
        }
    }

    @Override
    public synchronized KailleraGame createGame(KailleraUser user,
            String romName) throws CreateGameException, FloodException {
        if (!user.isLoggedIn()) {
        ...
        if (maxGameNameLength > 0
                && romName.trim().length() > maxGameNameLength) {
            log.warn(user + " create game denied: Rom Name Length > "
                    + maxGameNameLength);
            throw new CreateGameException(
        ...
        if (access == AccessManager.ACCESS_NORMAL) {
            if (createGameFloodTime > 0
                    && (System.currentTimeMillis() - ((KailleraUserImpl) user)
                            .getLastCreateGameTime()) < (createGameFloodTime * 1000)) {
            ...
            if (maxGames > 0 && getNumGames() >= maxGames) {
                log.warn(user + " create game denied: Over maximum of "
                        + maxGames + " current games!");
                throw new CreateGameException(
                        EmuLang.getString(
                                "KailleraServerImpl.CreateGameDeniedMaxGames",
                                maxGames));
            }
            ...
            for (int i = 0; i < chars.length; i++) {
                if (chars[i] < 32) {
                    log.warn(user
                            + " create game denied: Illegal characters in ROM name");
                    throw new CreateGameException(
            ...
            if (!accessManager.isGameAllowed(romName)) {
                log.warn(user
                        + " create game denied: AccessManager denied game: "
                        + romName);
                throw new CreateGameException(
        ...
        game = new KailleraGameImpl(gameID, romName, (KailleraUserImpl) user,
                this, gameBufferSize, gameTimeoutMillis, gameDesynchTimeouts);
        ...
        announce(EmuLang.getString(
                "KailleraServerImpl.UserCreatedGameAnnouncement",
                user.getName(), game.getRomName()), false, null);

        return game;
    }

    synchronized void closeGame(KailleraGame game, KailleraUser user)
    ...
    public boolean checkMe(KailleraUser user, String message) {
        ...
        // (System.currentTimeMillis() - ((KailleraUserImpl)
        // user).getLastChatTime()) < (chatFloodTime * 1000))
        // {
        ...
        if (access == AccessManager.ACCESS_NORMAL) {
            ...
            if (maxChatLength > 0 && message.length() > maxChatLength) {
                log.warn(user + " /me denied: Message Length > "
                        + maxChatLength);
                return false;
            }
        }
    ...
    public void announce(String announcement, boolean gamesAlso,
            ...
            if (gamesAlso) {// /msg and /me commands
                for (KailleraUserImpl kailleraUser : getUsers()) {
                    if (kailleraUser.isLoggedIn()) {
                        int access = accessManager.getAccess(user
                                .getConnectSocketAddress().getAddress());

                        if (access < AccessManager.ACCESS_ADMIN) {
                            if (!kailleraUser.searchIgnoredUsers(user
                                    .getConnectSocketAddress().getAddress()
                                    .getHostAddress())) {
                                kailleraUser.addEvent(new InfoMessageEvent(
                                        kailleraUser, announcement));
                            }
                        }
                        else {
                            kailleraUser.addEvent(new InfoMessageEvent(
                        ...
                        /*
                         * //SF MOD if(gamesAlso){ if(kailleraUser.getGame() !=
                         * null){ kailleraUser.getGame().announce(announcement,
                         * kailleraUser); Thread.yield(); } }
                         */
                    }
                }
            }
        ...
        for (KailleraUserImpl kailleraUser : getUsers()) {
            if (kailleraUser.isLoggedIn()) {
                kailleraUser.addEvent(new InfoMessageEvent(kailleraUser,
                        announcement));

                // SF MOD
                if (gamesAlso) {
                    if (kailleraUser.getGame() != null) {
                        kailleraUser.getGame().announce(announcement,
                                kailleraUser);
                        Thread.yield();
                    }
                }
    ...
    public void run() {
        ...
        try {
                    ...
                    synchronized (user) {
                        ...
                        if (user.isLoggedIn()) {
                            if (user.getGame() != null
                                ...
                                if (System.currentTimeMillis()
                                        - user.getGame().getStartTimeoutTime() > 15000) {
                        ...
                        if (!user.isLoggedIn()
                            ...
                            users.remove(user.getID());
                                ...
                                && (System.currentTimeMillis() - user
                                        .getLastKeepAlive()) > (keepAliveTimeout * 1000)) {
                            ...
                            try {
                                ...
                                && user.isLoggedIn()
                                && (System.currentTimeMillis() - user
                                        .getLastActivity()) > (idleTimeout * 1000)) {
                            log.info(user + " inactivity timeout!");
                            try {
                                quit(user,
                                        EmuLang.getString("KailleraServerImpl.ForcedQuitInactivityTimeout"));
                            }
                            catch (Exception e) {
                                log.error("Error forcing " + user
                                        + " quit for inactivity timeout!", e);
                            }
                        }
                        else if (user.isLoggedIn()
                                && access < AccessManager.ACCESS_NORMAL) {
                            ...
                            try {
                                ...
                                log.error("Error forcing " + user
                                        + " quit because banned!", e);
                            }
                        }
                        else if (user.isLoggedIn()
                                && access == AccessManager.ACCESS_NORMAL
                                && !accessManager.isEmulatorAllowed(user
                                        .getClientType())) {
                            log.info(user + ": emulator restricted!");
                            try {
                                quit(user,
                                        EmuLang.getString("KailleraServerImpl.ForcedQuitEmulatorRestricted"));
                            }
                            catch (Exception e) {
                                log.error("Error forcing " + user
                                        + " quit because emulator restricted!",
                                        e);
                            }
                        }
                    }
            ...
            if (!stopFlag) {
                log.fatal("KailleraServer thread caught unexpected exception: "
                        + e, e);
            }
        }
        finally {
