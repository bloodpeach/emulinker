...
public final class KailleraGameImpl implements KailleraGame {
    ...
    public synchronized int join(KailleraUser user) throws JoinGameException {
        ...
        if ((access < AccessManager.ACCESS_ELEVATED)
                && !aEmulator.equals("any")) {
            if (!aEmulator.equals(user.getClientType())) {
                log.warn(user
                        + " join game denied: Owner doesn't allow that emulator! " + this); 
                throw new JoinGameException(
                        "Join Game Denied: Owner only allows emulators that start with: " + aEmulator); 
            }
        }
        ...
        announce("Help: " + getServer().getReleaseInfo().getProductName()
                + " v" + getServer().getReleaseInfo().getVersionString() + ": "
                + getServer().getReleaseInfo().getReleaseDate()
                + " - Visit: www.God-Weapon.com", user);
        announce("************************", user);
        announce("Type /p2pon to ignore ALL server activity during gameplay.",
                user);
        announce(
                "This will reduce lag that you contribute due to a busy server.",
                user);
        ...
        /*
         * if(autoFireDetector != null) { if(autoFireDetector.getSensitivity() >
         * 0) {
         ...
         * , autoFireDetector.getSensitivity()));  } else {
    ...
    public synchronized void droppedPacket(KailleraUser user) {
        ...
        if (user.getPlayerNumber() > playerActionQueues.length) {
            log.info(this
                    + ": " + user + ": player desynched: dropped a packet! Also left the game already: KailleraGameImpl -> DroppedPacket");  
        }
    ...
    public void addData(KailleraUser user, int playerNumber, byte[] data)
        ...
        // int arraySize = (playerActionQueues.length * actionsPerMessage *
