...
public class KailleraAdminHttpServer implements Startable {
    ...
    public KailleraAdminHttpServer(Configuration config,
            ThreadPoolExecutor threadPool, ConnectController connectController,
            KailleraServer kailleraServer) throws Exception {
        ...
        ServletHttpContext context = (ServletHttpContext) appServer
                .getContext(jspDir);
     ...
     * 
     * @param context
     *            the context object that is to be configured
     * @param config
     *            the configuration file data from emulinker.xml
     * @throws IOException
     */
    private void setupSecurity(ServletHttpContext context, Configuration config)
            throws IOException {
        boolean isSecurity = config.getBoolean("adminserver.authenticate",
                false);
