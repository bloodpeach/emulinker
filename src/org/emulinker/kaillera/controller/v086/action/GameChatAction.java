...
public class GameChatAction implements V086Action, V086GameEventHandler {
    ...
    public void performAction(V086Message message,
        ...
        if (!(message instanceof GameChat_Request)) {
            throw new FatalActionException(
                    "Received incorrect instance of GameChat: " + message);
        }

        if (clientHandler.getUser() == null) {
            throw new FatalActionException(
                    "User does not exist: GameChatAction " + message);
        }

        if (((GameChat) message).getMessage().startsWith(
                ADMIN_COMMAND_ESCAPE_STRING)) {
            if (clientHandler.getUser().getAccess() >= AccessManager.ACCESS_ADMIN
                    || clientHandler.getUser().equals(
                            clientHandler.getUser().getGame().getOwner())) {
                try {
                    if (GameOwnerCommandAction.getInstance().isValidCommand(
                            ((GameChat) message).getMessage())) {
                        GameOwnerCommandAction.getInstance().performAction(
                                message, clientHandler);
                    }
                    else {
    ...
    private void checkCommands(V086Message message,
            V086Controller.V086ClientHandler clientHandler)
            throws FatalActionException {
        ...
        if (doCommand) {
            if (((GameChat) message).getMessage().equals("/msgon")) {
                ...
                KailleraUserImpl user = (KailleraUserImpl) clientHandler
                        .getUser();

                if (((GameChat) message).getMessage().equals("/p2pon")) {
                    clientHandler.getUser().setP2P(true);

                    if (clientHandler.getUser().getGame().getOwner()
                            .equals(clientHandler.getUser())) {
                        clientHandler.getUser().getGame().setP2P(true);

                        for (KailleraUserImpl u : clientHandler.getUser()
                                .getGame().getPlayers()) {
                            if (u.isLoggedIn()) {
                                u.getGame()
                                        .announce(
                    ...
                    user.getGame().announce(
                            "Failed P2P: /p2pon or /p2poff", user); //$NON-NLS-1$ 
                }

                return;
            }
            else if (((GameChat) message).getMessage().startsWith("/msg")) {
                KailleraUserImpl user1 = (KailleraUserImpl) clientHandler
                        .getUser();
                Scanner scanner = new Scanner(((GameChat) message).getMessage())
                        .useDelimiter(" "); //$NON-NLS-1$

                int access = clientHandler
                        .getUser()
                        .getServer()
                        .getAccessManager()
                        .getAccess(
                                clientHandler.getUser().getSocketAddress()
                                        .getAddress());
                ...
                try {
                    ...
                    KailleraUserImpl user = (KailleraUserImpl) clientHandler
                            .getUser().getServer().getUser(userID);
                    ...
                    if (user == null) {
                        ...
                        try {
                            clientHandler.send(new InformationMessage(
                    ...
                    if (user == clientHandler.getUser()) {
                        user1.getGame().announce(
                                "You can't private message yourself!", user1);
                        try {
                            clientHandler.send(new InformationMessage(
                                    clientHandler.getNextMessageNumber(),
                                    "server",
                    ...
                    if ((user.getMsg() == false)
                            || (user.searchIgnoredUsers(clientHandler.getUser()
                    ...
                    if (user1.getLastMsgID() != -1) {
                        try {
                            ...
                            if (user == null) {
                                user1.getGame().announce("User not found!",
                                        user1);
                                try {
                                    clientHandler.send(new InformationMessage(
                                            clientHandler
                ...
                try {
                    clientHandler
                            .send(new InformationMessage(clientHandler
                                    .getNextMessageNumber(), "server",
                                    clientHandler.getUser().getName()
                                            + " is now ignoring everyone!"));
                }
                catch (Exception e) {
                }
                return;
                ...
                try {
                    clientHandler.send(new InformationMessage(clientHandler
                            .getNextMessageNumber(), "server", clientHandler
                            .getUser().getName()
                            + " is now unignoring everyone!"));
                }
                catch (Exception e) {
                }
                return;
                ...
                try {
                    ...
                    if (user.getAccess() >= AccessManager.ACCESS_MODERATOR) {
                        try {
                            clientHandler.send(new InformationMessage(
                                    clientHandler.getNextMessageNumber(),
                                    "server", "You cannot ignore an admin!"));
                        }
                        catch (Exception e) {
                        }
                        return;
                    ...
                    if (user == clientHandler.getUser()) {
                        try {
                            clientHandler.send(new InformationMessage(
                                    clientHandler.getNextMessageNumber(),
                                    "server", "You can't ignore yourself!"));
                        }
                        catch (Exception e) {
                        }
                        return;
                    ...
                    if (clientHandler.getUser().findIgnoredUser(
                    ...
                    clientHandler.getUser().addIgnoredUser(
                            user.getConnectSocketAddress().getAddress()
                                    .getHostAddress());
                    user.getServer()
                            .announce(
                                    clientHandler.getUser().getName()
                                            + " is now ignoring <" + user.getName() + "> ID: " + user.getID(), false, null); //$NON-NLS-1$ //$NON-NLS-2$
                    return;
                }
                catch (NoSuchElementException e) {
                    KailleraUserImpl user = (KailleraUserImpl) clientHandler
                            .getUser();
                    user.getServer().announce(
                            "Ignore User Error: /ignore <UserID>", false, user); //$NON-NLS-1$ 
                    log.info("IGNORE USER ERROR: "
                            + user.getName()
                            + ": "
                            + clientHandler.getRemoteSocketAddress()
                                    .getHostName());
                    return;
                }
            }
                ...
                try {
                    ...
                    if (clientHandler.getUser().removeIgnoredUser(
                            user.getConnectSocketAddress().getAddress()
                                    .getHostAddress(), false) == true) {
                        user.getServer()
                                .announce(
                                        clientHandler.getUser().getName()
                                                + " is now unignoring <" + user.getName() + "> ID: " + user.getID(), false, null); //$NON-NLS-1$ //$NON-NLS-2$
                    }
                    else {
                        try {
                            clientHandler.send(new InformationMessage(
                    ...
                    user.getServer()
                            .announce(
                                    "Unignore User Error: /ignore <UserID>", false, user); //$NON-NLS-1$ 
                    log.info("UNIGNORE USER ERROR: "
                            + user.getName()
                            + ": "
                            + clientHandler.getRemoteSocketAddress()
                                    .getHostName());
                ...
                String announcement = ((GameChat) message).getMessage()
                        .substring(space + 1);

                if (announcement.startsWith(":")) {
                    announcement = announcement.substring(1); // this protects
                                                              // against
                                                              // people
                                                              // screwing up
                                                              // the emulinker
                ...
                announcement = "*" + clientHandler.getUser().getName() + " "
                        + m;

                for (KailleraUserImpl user : clientHandler.getUser().getGame()
                        .getPlayers()) {
            ...
            clientHandler.getUser().getGame()
                    .announce("Denied: Flood Control", clientHandler.getUser());
        }
    }
    ...
    public void handleEvent(GameEvent event,
        ...
        try {
            if (clientHandler.getUser().searchIgnoredUsers(
                    gameChatEvent.getUser().getConnectSocketAddress()
                ...
                if ((gameChatEvent.getUser().getAccess() < AccessManager.ACCESS_ADMIN)
                        && (gameChatEvent.getUser() != clientHandler.getUser())) {
                    return;
                }
            }
            ...
            clientHandler.send(new GameChat_Notification(clientHandler
                    .getNextMessageNumber(), gameChatEvent.getUser().getName(),
                    m));
        }
        catch (MessageFormatException e) {
            log.error(
                    "Failed to contruct GameChat_Notification message: "
                            + e.getMessage(), e);
        }
    }
}
