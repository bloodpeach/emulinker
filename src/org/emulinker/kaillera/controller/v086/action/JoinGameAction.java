    ...
    public void performAction(V086Message message,
        ...
        if (!(message instanceof JoinGame_Request)) {
            throw new FatalActionException(
                    "Received incorrect instance of JoinGame: " + message);
        }
        ...
        try {
            ...
            try {
                clientHandler.send(new InformationMessage(clientHandler
                        ...
                        .getString("JoinGameAction.JoinGameDenied")
                        + e.getMessage()));
                clientHandler.send(new QuitGame_Notification(clientHandler
                        .getNextMessageNumber(), clientHandler.getUser()
                        .getName(), clientHandler.getUser().getID()));
            }
                ...
                for (KailleraUser player : game.getPlayers()) {
                    if (!player.equals(thisUser)) {
                        if (player.getStealth() == false) {
                            players.add(new PlayerInformation.Player(player
                                    .getName(), player.getPing(), player
                                    .getID(), player.getConnectionType()));
                        }
                    }
                }

                clientHandler.send(new PlayerInformation(clientHandler
                        .getNextMessageNumber(), players));
            }

            if (user.getStealth() == false) {
                clientHandler.send(new JoinGame_Notification(clientHandler
                        .getNextMessageNumber(), game.getID(), 0, user
                        .getName(), user.getPing(), user.getID(), user
                        .getConnectionType()));
            }
        }
        catch (MessageFormatException e) {
            log.error(
                    "Failed to contruct JoinGame_Notification message: "
                            + e.getMessage(), e);
        }
    }
}
