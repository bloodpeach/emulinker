...
public class GameOwnerCommandAction implements V086Action {
    ...
    public void performAction(V086Message message,
        ...
        if (!user.equals(game.getOwner())
                && (user.getAccess() < AccessManager.ACCESS_SUPERADMIN)) {
        ...
        try {
            ...
            game.announce(
                    EmuLang.getString(
                            "GameOwnerCommandAction.CommandFailed", e.getMessage()), user); //$NON-NLS-1$
        }
    ...
    private void processHelp(String message, KailleraGameImpl game,
        ...
        game.announce(
                "/mute /unmute  <UserID> or /muteall or /unmuteall to mute player(s).", admin); //$NON-NLS-1$
        try {
            Thread.sleep(20);
        }
        catch (Exception e) {
        ...
        game.announce(
                "/swap <order> eg. 123..n {n = total # of players; Each slot = new player#}", admin); //$NON-NLS-1$
        try {
            Thread.sleep(20);
        }
        catch (Exception e) {
        ...
        game.announce(
                "/setemu <Emulator> To restrict the gameroom to this emulator!", admin); //$NON-NLS-1$
        try {
            Thread.sleep(20);
        }
        catch (Exception e) {
        ...
        game.announce(
                "/lagstat To check who has the most lag spikes or /lagreset to reset lagstat!", admin); //$NON-NLS-1$
        try {
            Thread.sleep(20);
        }
        catch (Exception e) {
        ...
        game.announce(
                "/samedelay {true | false} If you want both want to play at the same delay setting. Default is false.", admin); //$NON-NLS-1$
        try {
            Thread.sleep(20);
        }
        catch (Exception e) {
    ...
    private void autoFireHelp(KailleraGameImpl game, KailleraUserImpl admin) {
        ...
        game.announce(
                EmuLang.getString("GameOwnerCommandAction.HelpDisable"), admin); //$NON-NLS-1$
        try {
            Thread.sleep(20);
        }
        catch (Exception e) {
        ...
        game.announce(
                EmuLang.getString(
                        "GameOwnerCommandAction.HelpCurrentSensitivity", cur) + (cur == 0 ? (EmuLang.getString("GameOwnerCommandAction.HelpDisabled")) : ""), admin); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    }
    ...
    private void processDetectAutoFire(String message, KailleraGameImpl game,
        ...
        if (game.getStatus() != KailleraGame.STATUS_WAITING) {
            game.announce(
                    EmuLang.getString("GameOwnerCommandAction.AutoFireChangeDeniedInGame"), admin); //$NON-NLS-1$
            return;
        }
        ...
        game.announce(
                EmuLang.getString(
                        "GameOwnerCommandAction.HelpCurrentSensitivity", sensitivity) + (sensitivity == 0 ? (EmuLang.getString("GameOwnerCommandAction.HelpDisabled")) : ""), null); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    }
    ...
    private void processEmu(String message, KailleraGameImpl game,
        ...
        try {
            ...
            if (str.length() < 1) {
                admin.getGame().announce("Invalid Argument!", admin); //$NON-NLS-1$ 
                return;
            }
            ...
            if (admin.getServer().getAccessManager()
                ...
                admin.getGame()
                        .announce("Chat Denied: You're silenced!", admin); //$NON-NLS-1$ 
                return;
            }

            admin.getGame().setAEmulator(m);
            admin.getGame().announce(
                    "Owner has restricted the emulator to: " + m, null); //$NON-NLS-1$ 
            return;
        }
        catch (NoSuchElementException e) {
            admin.getGame().announce(
                    "Emulator Set Error: /setemu <Emulator>", admin); //$NON-NLS-1$ 
        }
    }
    ...
    private void processNum(String message, KailleraGameImpl game,
            ...
            V086Controller.V086ClientHandler clientHandler)
            throws ActionException, MessageFormatException {
        admin.getGame().announce(game.getNumPlayers() + " in the room!", admin); //$NON-NLS-1$ 
    }
    ...
    private void processMute(String message, KailleraGameImpl game,
        ...
        try {
            ...
            if (str.equals("/muteall")) {
                ...
                admin.getGame().announce("All players have been muted!", null); //$NON-NLS-1$ 
                return;
            }
            ...
            if (user == null) {
                admin.getGame().announce("Player doesn't exist!", admin); //$NON-NLS-1$ 
                return;
            }

            if ((user.getAccess() >= AccessManager.ACCESS_ADMIN)
                    && (admin.getAccess() != AccessManager.ACCESS_SUPERADMIN)) {
                user.getGame().announce("You can't mute an Admin", admin); //$NON-NLS-1$ 
                return;
            }

            if (user == clientHandler.getUser()) {
                user.getGame().announce("You can't mute yourself!", admin); //$NON-NLS-1$ 
                return;
            }
            ...
            user.getGame().announce(
                    "Mute Player Error: /mute <PlayerNumber>", admin); //$NON-NLS-1$ 
        }
    }
    ...
    private void processUnmute(String message, KailleraGameImpl game,
        ...
        try {
            ...
            if (str.equals("/unmuteall")) {
                ...
                admin.getGame()
                        .announce("All players have been unmuted!", null); //$NON-NLS-1$ 
                return;
            }
            ...
            if ((user.getAccess() >= AccessManager.ACCESS_ADMIN)
                    && (admin.getAccess() != AccessManager.ACCESS_SUPERADMIN)) {
                user.getGame().announce("You can't unmute an Admin", admin); //$NON-NLS-1$ 
                return;
            }

            if (user == clientHandler.getUser()) {
                user.getGame().announce("You can't unmute yourself!", admin); //$NON-NLS-1$ 
                return;
            }
            ...
            KailleraUserImpl user = (KailleraUserImpl) clientHandler.getUser();
            user.getGame().announce(
                    "Unmute Player Error: /unmute <PlayerNumber>", admin); //$NON-NLS-1$ 
        }
    }
    ...
    private void processSwap(String message, KailleraGameImpl game,
        ...
        if (game.getStatus() != KailleraGame.STATUS_PLAYING) {
            game.announce(
                    "Failed: wap Players can only be used during gameplay!",
                    admin);
            return;
        }
    ...
    private void processKick(String message, KailleraGameImpl game,
        ...
        try {
            ...
            if (str.equals("/kickall")) {
                ...
                admin.getGame().announce("All players have been kicked!", null); //$NON-NLS-1$ 
                return;
            }
