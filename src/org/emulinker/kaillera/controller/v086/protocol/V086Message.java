...
public abstract class V086Message extends ByteBufferMessage {
    ...
    protected V086Message(int number) throws MessageFormatException {
        if (number < 0 || number > 0xFFFF) {
            throw new MessageFormatException("Invalid " + getDescription()
                    + " format: Invalid message number: " + number);
        }
    ...
    public void writeTo(ByteBuffer buffer) {
        ...
        if (len > buffer.remaining()) {
            ...
            // there no realistic reason to use unsigned here since a single
            // packet can't be that large
            buffer.mark();
    ...
    public static V086Message parse(int messageNumber, int messageLength,
            ByteBuffer buffer) throws ParseException, MessageFormatException {
        ...
        if (message.getLength() != messageLength) {
            // throw new ParseException("Bundle contained length " +
            // messageLength + " !=  parsed length " + message.getLength());
            log.debug("Bundle contained length " + messageLength
