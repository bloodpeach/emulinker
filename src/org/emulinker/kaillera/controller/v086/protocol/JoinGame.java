...
public abstract class JoinGame extends V086Message {
    ...
    public JoinGame(int messageNumber, int gameID, int val1, String userName,
            long ping, int userID, byte connectionType)
    ...
    public static JoinGame parse(int messageNumber, ByteBuffer buffer)
        ...
        if (b != 0x00) {
            throw new MessageFormatException("Invalid format: byte 0 = "
                    + EmuUtil.byteToHex(b));
        }
        ...
        if (userName.length() == 0 && ping == 0 && userID == 0xFFFF) {
            ...
            return new JoinGame_Notification(messageNumber, gameID, val1,
                    userName, ping, userID, connectionType);
        }
    }
}
