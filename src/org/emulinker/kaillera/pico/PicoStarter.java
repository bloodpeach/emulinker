...
public class PicoStarter {
    ...
    /**
     * Main entry point for the EmuLinker Kaillera server. This method accepts
     * no arguments. It starts the pico container which reads its configuration
     * from components.xml. The server components, once started, read their
     * configuration information from emulinker.xml. Each of those files will be
    ...
    public static void main(String args[]) {
        try {
            ...
            log.fatal(e);
            System.out
                    .println("Failed to start! A required propery is missing: "
                            + e.getMessage());
            System.exit(1);
        }
            ...
            log.fatal(e);
            System.out
                    .println("Failed to start! A configuration parameter is incorrect: "
                            + e.getMessage());
            System.exit(1);
        }
            ...
            log.fatal("EmuLinker server failed to start!");
            log.fatal(e);
            System.out.println("Failed to start! A server is already running: "
                    + e.getMessage());
            System.exit(1);
        }
        catch (Throwable e) {
            ...
            log.fatal(e);
            System.err
                    .println("Failed to start! Caught unexpected error, stacktrace follows: "
                            + e.getMessage());
    ...
    private PicoStarter() throws Exception {
        ...
        ReleaseInfo releaseInfo = (ReleaseInfo) container
                .getComponentInstanceOfType(ReleaseInfo.class);
