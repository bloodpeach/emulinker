...
public class KailleraMasterUpdateTask implements MasterListUpdateTask {
    ...
    public void touchMaster() {
        ...
        params[1] = new NameValuePair("port",
                Integer.toString(connectController.getBindPort()));
        params[2] = new NameValuePair("nbusers",
                Integer.toString(kailleraServer.getNumUsers()));
        params[3] = new NameValuePair("maxconn",
                Integer.toString(kailleraServer.getMaxUsers()));

        params[4] = new NameValuePair("version", releaseInfo.getVersionString());
        params[5] = new NameValuePair("nbgames",
        ...
        HttpMethod kailleraTouch = new GetMethod(
                "http://www.kaillera.com/touch_server.php");
        kailleraTouch.setQueryString(params);
        kailleraTouch.setRequestHeader("Kaillera-games",
                createdGames.toString());
        kailleraTouch.setRequestHeader("Kaillera-wgames",
                waitingGames.toString());

        try {
            int statusCode = httpClient.executeMethod(kailleraTouch);
            if (statusCode != HttpStatus.SC_OK) {
                log.error("Failed to touch Kaillera Master: "
                        + kailleraTouch.getStatusLine());
            }
            else {
