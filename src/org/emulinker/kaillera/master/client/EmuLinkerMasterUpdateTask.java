...
public class EmuLinkerMasterUpdateTask implements MasterListUpdateTask {
    ...
    public void touchMaster() {
        ...
        params[1] = new NameValuePair("connectAddress",
                publicInfo.getConnectAddress());
        params[2] = new NameValuePair("location", publicInfo.getLocation());
        params[3] = new NameValuePair("website", publicInfo.getWebsite());
        params[4] = new NameValuePair("port",
                Integer.toString(connectController.getBindPort()));
        params[5] = new NameValuePair("connectCount",
                Integer.toString(connectController.getConnectCount()));
        params[6] = new NameValuePair("numUsers",
                Integer.toString(kailleraServer.getNumUsers()));
        params[7] = new NameValuePair("maxUsers",
                Integer.toString(kailleraServer.getMaxUsers()));
        params[8] = new NameValuePair("numGames",
                Integer.toString(kailleraServer.getNumGames()));
        params[9] = new NameValuePair("maxGames",
                Integer.toString(kailleraServer.getMaxGames()));
        params[10] = new NameValuePair("version", releaseInfo.getProductName()
                + " v" + releaseInfo.getVersionString());
        params[11] = new NameValuePair("build", Integer.toString(releaseInfo
                .getBuildNumber()));
        params[12] = new NameValuePair("isWindows", Boolean.toString(EmuUtil
                .systemIsWindows()));
        ...
        try {
            ...
            if (statusCode != HttpStatus.SC_OK) {
                log.error("Failed to touch Kaillera Master: "
                        + meth.getStatusLine());
            }
            else {
