...
public class PlayerActionQueue {
    ...
    public PlayerActionQueue(int playerNumber, KailleraUserImpl player,
            int numPlayers, int gameBufferSize, int gameTimeoutMillis,
            boolean capture) {
         ...
         * FileOutputStream("test.cap")); } catch(Exception e) {
         * e.printStackTrace(); } }
         */
    }
    ...
    public void getAction(int playerNumber, byte[] actions, int location,
        ...
        /*
         * In EmuSF the following is present, but seems obviously wrong //Why
         * would we thow error only if synced!?!?! if (!synched) return; throw
         * new PlayerTimeoutException(thisPlayerNumber, thisPlayer);
         */
    ...
    private int getSize(int playerNumber) {
        return (tail + gameBufferSize - heads[playerNumber - 1])
                % gameBufferSize;
    }
}
