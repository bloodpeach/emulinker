...
public class Trivia implements Runnable {
    ...
    public Trivia(KailleraServerImpl server) {
        ...
        try {
            server.announce(
                    "<Unscramble> " + "Loading GalaxyScramble Words...", false,
                    null);
             ...
             */
            while (scan.hasNext()) { // read line by line
        ...
        for (int j = 0; j < unscramblewords.size(); j++) {
            ...
            for (int k = 0; k < tempword.length(); k++) {
                int r = generator.nextInt(tempword.length()); // generate two
                                                              // random
                                                              // integers for
                                                              // place values
            ...
            Questions tempQuestion = new Questions(scrambledword,
                    unscramblewords.get(j).toString());
            questions.add(tempQuestion);
            questions_num.add(j);
        }
        server.announce("<Unscramble> " + questions.size()
                + " words have been loaded!", false, null);
        server.announce(
                "<Unscramble> " + "GalaxyUnscramble will begin in 10s!", false,
                null);
    }
        ...
        while (!exitThread) {
            if (!triviaPaused) {
                if (newQuestion) {
                    ...
                    hint1 = true;
                    hint2 = false;
                    Questions thisquestion = questions.get(questions_count);
                    server.announce("<Unscramble> " + "Unscramble this word: "
                            + thisquestion.getQuestion(), false, null);
                    if (!answered) {
                        try {
                            Thread.sleep(10000);
                        }
                    ...
                    if (!answered) {
                        server.announce("<Unscramble> " + "35 seconds left...",
                                false, null);
                        try {
                            Thread.sleep(5000);
                        }
                ...
                if (hint1 && !answered) {
                    ...
                    hint1 = false;
                    hint2 = true;
                    Questions tempquestion2 = questions.get(questions_count);
                    ...
                    server.announce("<Unscramble> " + "Hint 1: " + answer,
                            false, null);
                    if (!answered) {
                        try {
                            Thread.sleep(10000);
                    ...
                    if (!answered) {
                        server.announce("<Unscramble> " + "20 seconds left...",
                                false, null);
                        try {
                            Thread.sleep(5000);
                        }
                ...
                if (hint2 && !answered) {
                    ...
                    server.announce("<Unscramble> " + "Hint 2: " + answer,
                            false, null);
                    if (!answered) {
                        try {
                            Thread.sleep(10000);
                    ...
                    if (!answered) {
                        server.announce("<Unscramble> " + "5 seconds left...",
                                false, null);
                        try {
                            Thread.sleep(5000);
                        }
    ...
    public boolean isCorrect(String message) {
        ...
        if (message.toLowerCase().equals(
                tempquestion2.getAnswer().toLowerCase())) {
            answered = true;
            return true;
        }
        else {
            for (int i = 0; i < numbers0.length; i++) {
                if (tempquestion2.getAnswer().toLowerCase()
                        .contains(numbers0[i])) {
                    message = message.replace(numbers1[i], numbers0[i]);
                }
                else if (tempquestion2.getAnswer().toLowerCase()
                        .contains(numbers1[i])) {
    ...
    public void addScore(String nick, String ip, String answer) {
            ...
            if (tempscore.getIP().equals(ip)) {
                if (ip_streak.equals(ip)) {
                    ...
                    if (score_streak > 1) {
                        ...
                        server.announce("<Unscramble> ***" + nick
                                + " has answered " + score_streak
                                + " in a row!***", false, null);
                ...
                Questions tempquestion2 = questions.get(questions_count);
                server.announce("<Unscramble> Congratulations " + nick
                        + "! You have answered the question correctly with ("
                        + answer + ")! Your score is: " + s, false, null);
                return;
            }
        }
        ...
        ip_streak = ip;
        server.announce("<Unscramble> Congratulations " + nick
                + "! You have answered the question correctly with (" + answer
                + ")! Your score is: 1", false, null);
    }
    ...
    public void displayHighScores(boolean winner) {
        ...
        server.announce("<Unscramble> " + "(Top 3 Scores of " + scores.size()
                + ") " + str, false, null);
    }
    ...
    public void saveScores(boolean display) {
        try {
            ...
            if (display) {
                server.announce("<Unscramble> "
                        + "GalaxyUnscramble Scores were Saved Successfully!",
                        false, null);
            }
        }
        catch (Exception e) {
            server.announce("<Unscramble> "
                    + "Error Saving GalaxyUnscramble Scores!", false, null);
        }
    }
}
