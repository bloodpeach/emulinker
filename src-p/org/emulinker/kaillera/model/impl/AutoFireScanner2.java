...
public class AutoFireScanner2 implements AutoFireDetector {
    ...
    private static int SENSITIVITY_TABLE[][] = { { 0, 0 }, // 0 means disable
                                                           // autofire detect
    ...
    protected class ScanningJob implements Runnable {
        ...
        protected synchronized void addData(byte[] data, int bytesPerAction) {
            if (pos + data.length >= sizeLimit) {
                ...
                // log.debug("tail="+tail);
                System.arraycopy(data, firstSize, buffer[tail], 0, data.length
                        - firstSize);
        ...
        public void run() {
            ...
            try {
                while (size > 0 && !stopFlag) {
                    ...
                    synchronized (this) {
                        data = buffer[head];
                        // log.debug("Scanning " + data.length +
                        // " bytes from buffer position " + head);
                    ...
                    for (int i = 0; i < actionCount; i++) {
                        System.arraycopy(data, i * bytesPerAction, thisAction,
                                0, bytesPerAction);

                        // log.debug("thisAction=" +
                        // EmuUtil.bytesToHex(thisAction) + " actionA=" +
                        // EmuUtil.bytesToHex(actionA) + " aCount=" + aCount +
                        // " actionB=" + EmuUtil.bytesToHex(actionB) +
                        // " bCount=" + bCount + " aSequence=" + aSequence +
                        // " aSequenceCount=" + aSequenceCount + " bSequence=" +
                        // bSequence + " bSequenceCount=" + bSequenceCount);

                        if (aCount == 0) {
                            System.arraycopy(thisAction, 0, actionA, 0,
                                    bytesPerAction);
                            aPos = i;
                            aCount = 1;
                            aSequence = 1;
                            ...
                            System.arraycopy(thisAction, 0, actionB, 0,
                                    bytesPerAction);
                            bPos = i;
                            bCount = 1;
                            bSequence = 1;
                        ...
                        System.arraycopy(thisAction, 0, lastAction, 0,
                                bytesPerAction);

                        // if(aSequenceCount >= 3 && bSequenceCount >= 3 &&
                        // !stopFlag)
                        // {
                        // log.debug("thisAction=" +
                        // EmuUtil.bytesToHex(thisAction) + " actionA=" +
                        ...
                        if (aSequenceCount >= minReps
                                && bSequenceCount >= minReps && !stopFlag) {
                            KailleraGameImpl gameImpl = (KailleraGameImpl) game;
                            gameImpl.announce(EmuLang.getString(
                                    "AutoFireScanner2.AutoFireDetected",
                                    user.getName()), user); // user - null in SF
                            ...
                            // + " actionB=" + EmuUtil.bytesToHex(actionB) +
                            // " bCount=" + bCount + " aSequence=" + aSequence +
                            // " aSequenceCount=" + aSequenceCount +
                            // " bSequence=" + bSequence + " bSequenceCount=" +
                            // bSequenceCount);
                            break;
                        }
                    }
                ...
                log.error(
                        "AutoFireScanner2 thread for " + user + " caught exception!", e); //$NON-NLS-1$ //$NON-NLS-2$
            }
            finally {
                synchronized (this) {
                    running = false;
