...
public class AccessManager2 implements AccessManager, Startable, Runnable {
    ...
    public synchronized void start() {
        ...
        log.debug("AccessManager2 thread starting (ThreadPool:"
    ...
    public void run() {
        ...
        try {
            ...
            if (!stopFlag) {
                log.fatal("AccessManager2 thread caught unexpected exception: "
                        + e, e);
            }
        }
        finally {
    ...
    private synchronized void checkReload() {
        if (accessFile != null
                && accessFile.lastModified() > lastLoadModifiedTime) {
            loadAccess();
        }
    }
    ...
    private synchronized void loadAccess() {
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (st.countTokens() < 3) {
                    log.error("Failed to load access line, too few tokens: "
                            + line);
                    continue;
                }
                ...
                try {
                    ...
                    log.error("Failed to load access line: " + e.getMessage()
                            + ": " + line);
                }
            }
    ...
    @Override
    public synchronized boolean clearTempSilence(InetAddress address) {
        String userAddress = address.getHostAddress();
        boolean found = false;
    ...
    protected class UserAccess {
        ...
        protected UserAccess(StringTokenizer st) throws Exception {
            ...
            if (accessStr.equals("normal")) {
                ...
                throw new AccessException("Unrecognized access token: "
                        + accessStr);
            }
            ...
            while (pt.hasMoreTokens()) {
                ...
                if (pat.startsWith("dns:")) {
                    if (pat.length() <= 5) {
                        throw new AccessException("Malformatted DNS entry: "
                                + s);
                    }
                    ...
                    try {
                        ...
                        log.debug("Resolved " + hostName + " to "
                                + a.getHostAddress());
                    }
                    catch (Exception e) {
                        log.warn("Failed to resolve DNS entry to an address: "
        ...
        protected synchronized void refreshDNS() {
            ...
            for (String hostName : hostNames) {
                try {
                    ...
                    log.debug("Failed to resolve DNS entry to an address: "
                            + hostName, e);
                }
            }
        }
    ...
    protected class EmulatorAccess {
        ...
        protected EmulatorAccess(StringTokenizer st) throws Exception {
            ...
            while (pt.hasMoreTokens()) {
                patterns.add(new WildcardStringPattern(pt.nextToken()
                        .toLowerCase()));
            }
        }
    ...
    /*
     * bloodpeach refactor: Silence, TempElevated, TempAdmin and TempBan were
     * all literally the same class with different names. made the impied parent
     * class to spare myself the guilt of writing such bad code. will do real
     * refactor later
     */
    protected class AccessStatus {
        protected List<WildcardStringPattern> patterns;
        protected long startTime;
        protected int minutes;

        protected AccessStatus(String accessStr, int nminutes) {
            patterns = new ArrayList<WildcardStringPattern>();
    ...
    protected class Silence extends AccessStatus {
        public Silence(String accessStr, int nminutes) {
            super(accessStr, nminutes);
        }
    }

    protected class TempElevated extends AccessStatus {
        public TempElevated(String accessStr, int nminutes) {
            super(accessStr, nminutes);
        }
    }

    protected class TempAdmin extends AccessStatus {
        public TempAdmin(String accessStr, int nminutes) {
            super(accessStr, nminutes);
        }
    }

    protected class TempBan extends AccessStatus {
        public TempBan(String accessStr, int nminutes) {
            super(accessStr, nminutes);
        }
    }
