...
public class ChatAction implements V086Action, V086ServerEventHandler {
    ...
    public void performAction(V086Message message,
        ...
        if (!(message instanceof Chat_Request)) {
            throw new FatalActionException(
                    "Received incorrect instance of Chat: " + message);
        }

        if (((Chat) message).getMessage().startsWith(
                ADMIN_COMMAND_ESCAPE_STRING)) {
            if (clientHandler.getUser().getAccess() > AccessManager.ACCESS_ELEVATED) {
                try {
                    if (AdminCommandAction.getInstance().isValidCommand(
                            ((Chat) message).getMessage())) {
                        AdminCommandAction.getInstance().performAction(message,
                                clientHandler);
        ...
        try {
            ...
            try {
                clientHandler.send(new InformationMessage(clientHandler
                        ...
                        "ChatAction.ChatDenied", e.getMessage())));
            }
    ...
    private void checkCommands(V086Message message,
        ...
        if (doCommand) {
            ...
            if (((Chat) message).getMessage().equals("/alivecheck")) {
                ...
                ReleaseInfo releaseInfo = clientHandler.getUser().getServer()
                        .getReleaseInfo();
                try {
                    clientHandler.send(new InformationMessage(clientHandler
                ...
                String announcement = ((Chat) message).getMessage().substring(
                        space + 1);
                ...
                if (clientHandler.getUser().getServer()
                        .checkMe(clientHandler.getUser(), announcement)) {
                    ...
                    KailleraUserImpl user1 = (KailleraUserImpl) clientHandler
                            .getUser();
                    clientHandler.getUser().getServer()
                            .announce(announcement, true, user1);
                }
            }
            else if (((Chat) message).getMessage().startsWith("/msg")) {
                KailleraUserImpl user1 = (KailleraUserImpl) clientHandler
                        .getUser();
                Scanner scanner = new Scanner(((Chat) message).getMessage())
                    ...
                    if (user.getAccess() >= AccessManager.ACCESS_MODERATOR) {
                        try {
                            clientHandler.send(new InformationMessage(
                                    clientHandler.getNextMessageNumber(),
                                    "server",
                                    "You cannot ignore an admin/moderator!"));
                        }
                        catch (Exception e) {
                        }
                        return;
            ...
            else if (((Chat) message).getMessage().equals("/rules")) {
                ...
                try {
                    clientHandler.send(new InformationMessage(clientHandler
                            .getNextMessageNumber(), "server", contents
                            .toString()));
                }
                catch (Exception e) {
                }
                ...
                try {
                    clientHandler
                            .send(new InformationMessage(clientHandler
                                    .getNextMessageNumber(), "server",
                                    "/msg <UserID> <msg> to PM somebody. /msgon or /msgoff to turn on | off."));
                }
                catch (Exception e) {
                }
                try {
                ...
                try {
                    clientHandler.send(new InformationMessage(clientHandler
                            .getNextMessageNumber(), "server",
                            "/rules for the server's rule list."));
                }
                catch (Exception e) {
                }
                try {
                ...
                int foundCount = 0;
                String str = (((Chat) message).getMessage()
                        .substring(space + 1));
                ...
                userN.getServer().announce(
                        "Unknown Command: " + ((Chat) message).getMessage(),
                        false, userN);
            }
        }
        else {
    ...
    public void handleEvent(ServerEvent event,
        ...
        try {
            ...
            clientHandler.send(new Chat_Notification(clientHandler
                    .getNextMessageNumber(), chatEvent.getUser().getName(), m));
        }
        catch (MessageFormatException e) {
            log.error(
                    "Failed to contruct Chat_Notification message: "
                            + e.getMessage(), e);
        }
    }
}
