...
public class ACKAction implements V086Action, V086UserEventHandler {
    ...
    public void performAction(V086Message message,
        ...
        if (clientHandler.getSpeedMeasurementCount() > numAcksForSpeedTest) {
            ...
            try {
                ...
                try {
                    clientHandler.send(new ConnectionRejected(clientHandler
                            .getNextMessageNumber(), user.getName(), user
                            .getID(), e.getMessage()));
                }
                ...
                throw new FatalActionException("Login failed: "
                        + e.getMessage());
            }
        }
        else {
    ...
    public void handleEvent(UserEvent event,
        ...
        try {
            for (KailleraUser user : server.getUsers()) {
                if (user.getStatus() != KailleraUser.STATUS_CONNECTING
                        && !user.equals(thisUser)) {
                    users.add(new ServerStatus.User(user.getName(), user
                            .getPing(), (byte) user.getStatus(), user.getID(),
        ...
        try {
            for (KailleraGame game : server.getGames()) {
                ...
                games.add(new ServerStatus.Game(game.getRomName(),
                        game.getID(), game.getClientType(), game.getOwner()
                                .getName(), (num + "/" + game.getMaxUsers()),
                        (byte) game.getStatus()));
            }
        }
        catch (MessageFormatException e) {
        ...
        // Here I am attempting to fix the inherent Kaillera protocol bug that
        // occurs when there are a large number of users
        // and/or games on the server. The size of the ServerStatus packet can
        // be very large, and depending on the router
        // settings or os config, the packet size exceeds a UDP/IP limit and
        // gets dropped. This results in the user getting
        // half logged-in, in a weird state.

        // I am attempting to fix this by breaking the ServerStatus message up
        // into multiple packets. I'm shooting for a max
        // packet size of 1500 bytes, but since kaillera sends 3 messages per
        // packet, the max size for a single message should be 500
        ...
        while (!users.isEmpty()) {
            ...
            if ((counter + user.getLength()) >= 300) {
                sendServerStatus(clientHandler, usersSubList, gamesSubList,
                        counter);
    ...
    private void sendServerStatus(
            V086Controller.V086ClientHandler clientHandler,
            List<ServerStatus.User> users, List<ServerStatus.Game> games,
            int counter) {
        ...
        log.debug("Sending ServerStatus to " + clientHandler.getUser() + ": "
                + users.size() + " users, " + games.size() + " games in "
                + counter + " bytes, games: " + sb.toString());
        try {
            clientHandler.send(new ServerStatus(clientHandler
                    .getNextMessageNumber(), users, games));
        }
        catch (MessageFormatException e) {
