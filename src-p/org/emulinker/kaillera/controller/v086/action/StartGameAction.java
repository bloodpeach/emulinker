...
public class StartGameAction implements V086Action, V086GameEventHandler {
    ...
    public void performAction(V086Message message,
        ...
        if (!(message instanceof StartGame_Request)) {
            throw new FatalActionException(
                    "Received incorrect instance of StartGame: " + message);
        }
        ...
        try {
            ...
            try {
                clientHandler.send(new GameChat_Notification(clientHandler
                        .getNextMessageNumber(), "Error", e.getMessage()));
            }
            catch (MessageFormatException ex) {
                log.error("Failed to contruct GameChat_Notification message: "
                        + e.getMessage(), e);
            }
        }
    }
    ...
    public void handleEvent(GameEvent event,
        ...
        try {
            ...
            clientHandler.getUser().setTempDelay(
                    game.getDelay() - clientHandler.getUser().getDelay());
            ...
            clientHandler.send(new StartGame_Notification(clientHandler
                    .getNextMessageNumber(), (short) delay,
                    (byte) playerNumber, (byte) game.getNumPlayers()));
        }
        catch (MessageFormatException e) {
            log.error(
                    "Failed to contruct StartGame_Notification message: "
                            + e.getMessage(), e);
        }
    }
}