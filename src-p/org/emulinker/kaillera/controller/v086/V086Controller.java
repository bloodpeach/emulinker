...
public class V086Controller implements KailleraServerController {
    ...
    private int MAX_BUNDLE_SIZE = 9; // 5 in Vanilla
    ...
    public V086Controller(KailleraServer server, ThreadPoolExecutor threadPool,
        ...
        this.clientTypes = config
                .getStringArray("controllers.v086.clientTypes.clientType");
        ...
        for (int i = portRangeStart; i <= (portRangeStart
                + server.getMaxUsers() + extraPorts); i++) {
        ...
        if (bufferSize <= 0) {
            throw new ConfigurationException(
                    "controllers.v086.bufferSize must be > 0");
        }
        ...
        serverEventHandlers.put(GameCreatedEvent.class,
                CreateGameAction.getInstance());
        serverEventHandlers.put(UserJoinedEvent.class,
                LoginAction.getInstance());
        serverEventHandlers.put(GameClosedEvent.class,
                CloseGameAction.getInstance());
        serverEventHandlers.put(UserQuitEvent.class, QuitAction.getInstance());
        serverEventHandlers.put(GameStatusChangedEvent.class,
                GameStatusAction.getInstance());

        // setup the game event handlers
        gameEventHandlers.put(UserJoinedGameEvent.class,
                JoinGameAction.getInstance());
        gameEventHandlers.put(UserQuitGameEvent.class,
                QuitGameAction.getInstance());
        gameEventHandlers.put(GameStartedEvent.class,
                StartGameAction.getInstance());
        gameEventHandlers
                .put(GameChatEvent.class, GameChatAction.getInstance());
        gameEventHandlers.put(AllReadyEvent.class,
                UserReadyAction.getInstance());
        gameEventHandlers
                .put(GameDataEvent.class, GameDataAction.getInstance());
        gameEventHandlers.put(UserDroppedGameEvent.class,
                DropGameAction.getInstance());
        gameEventHandlers.put(GameDesynchEvent.class,
                GameDesynchAction.getInstance());
        gameEventHandlers.put(PlayerDesynchEvent.class,
                PlayerDesynchAction.getInstance());
        gameEventHandlers
                .put(GameInfoEvent.class, GameInfoAction.getInstance());
        gameEventHandlers.put(GameTimeoutEvent.class,
                GameTimeoutAction.getInstance());
        gameEventHandlers.put(GameTimeoutEvent.class,
                GameTimeoutAction.getInstance());

        // setup the user event handlers
        userEventHandlers.put(ConnectedEvent.class, ACKAction.getInstance());
        userEventHandlers.put(InfoMessageEvent.class,
                InfoMessageAction.getInstance());
    }
    ...
    @Override
    public int newConnection(InetSocketAddress clientSocketAddress,
            String protocol) throws ServerFullException, NewConnectionException {
        if (!isRunning) {
            throw new NewConnectionException("Controller is not running");
        }

        V086ClientHandler clientHandler = new V086ClientHandler(
                clientSocketAddress);
        KailleraUser user = server.newConnection(clientSocketAddress, protocol,
                clientHandler);
        ...
        while (bindAttempts++ < 5) {
            ...
            if (portInteger == null) {
                ...
                try {
                    ...
                    boundPort = port;
                    break; // break not in decompile. dont think it does what
                           // author intended
                }
                catch (BindException e) {
                    log.error("Failed to bind to port " + port + " for: "
                            + user + ": " + e.getMessage(), e);
                    log.debug(toString() + " returning port " + port
    ...
    public class V086ClientHandler extends PrivateUDPServer implements
        ...
        // private LinkedList<V086Message> lastMessages = new
        // LinkedList<V086Message>();
        private LastMessageBuffer lastMessageBuffer = new LastMessageBuffer(
                MAX_BUNDLE_SIZE);
        ...
        public void resetGameDataCache() {
            ...
            // serverCache = new ClientGameDataCache(256); //SF Mod?
        }
        ...
        public void start(KailleraUser user) {
            this.user = user;
            log.debug(toString() + " thread starting (ThreadPool:"
             ...
             * (System.currentTimeMillis() - s) < 1000) { try {
            ...
            log.debug(toString() + " thread started (ThreadPool:"
                    + threadPool.getActiveCount() + "/"
                    + threadPool.getPoolSize() + ")");
        ...
        protected void handleReceived(ByteBuffer buffer) {
            ...
            try {
                ...
                buffer.rewind();
                log.warn(
                        toString() + " received invalid message: "
                                + EmuUtil.dumpBuffer(buffer), e);
                return;
            }
            ...
            if (inBundle.getNumMessages() == 0) {
                log.debug(toString() + " received bundle of "
                        + inBundle.getNumMessages() + " messages from " + user);
                clientRetryCount++;
            ...
            try {
                synchronized (inSynch) {
                    // read the bundle from back to front to process the oldest
                    // messages first
                        ...
                        for (int i = (inBundle.getNumMessages() - 1); i >= 0; i--) {
                            if (messages[i].getNumber() > lastMessageNumber) {
                                ...
                                if ((prevMessageNumber + 1) != lastMessageNumber) {
                                    log.warn(user + " dropped a packet! ("
                                            + prevMessageNumber + " to "
                                            + lastMessageNumber + ")");
                                    user.droppedPacket();
                                }
                                ...
                                if (action == null) {
                                    log.error("No action defined to handle client message: "
                                            + messages[i]);
                                    continue; // continue not in decompile. This
                                              // code is structures poorlhy
                                              // and i dont think the continue
                                              // has desired effect.
                                }
                ...
                log.warn(toString() + " fatal action, closing connection: "
                        + e.getMessage());
                Thread.yield();
                stop();
            }
        ...
        public void actionPerformed(KailleraEvent event) {
            if (event instanceof GameEvent) {
                V086GameEventHandler eventHandler = gameEventHandlers.get(event
                        .getClass());
                ...
                V086ServerEventHandler eventHandler = serverEventHandlers
                        .get(event.getClass());
                ...
                V086UserEventHandler eventHandler = userEventHandlers.get(event
                        .getClass());
        ...
        public void resend(int timeoutCounter) {
            synchronized (outSynch) {
                ...
                // if ((System.currentTimeMillis() - lastResend) >
                // (user.getPing()*3))
                if ((System.currentTimeMillis() - lastResend) > server
                        .getMaxPing()) {
                    ...
                    log.debug(this + ": resending last " + numToSend
                            + " messages");
                    send(null, numToSend);
