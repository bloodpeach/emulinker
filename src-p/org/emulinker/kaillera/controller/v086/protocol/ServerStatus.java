...
public class ServerStatus extends V086Message {
    ...
    public String toString() {
        ...
        sb.append(getInfoString() + "[users=" + users.size() + " games="
                + games.size() + "]");
    ...
    public static ServerStatus parse(int messageNumber, ByteBuffer buffer)
        ...
        if (b != 0x00) {
            throw new MessageFormatException("Invalid " + DESC
                    + " format: byte 0 = " + EmuUtil.byteToHex(b));
        }
        ...
        for (int j = 0; j < numGames; j++) {
            ...
            games.add(new Game(romName.toString(), gameID, clientType,
                    userName, players, status));
        }
    ...
    public static class User {
        ...
        public User(String userName, long ping, byte status, int userID,
            ...
            if (ping < 0 || ping > 2048) {
                throw new MessageFormatException("Invalid " + DESC
                        + " format: ping out of acceptable range: " + ping);
            }

            if (status < 0 || status > 2) {
                throw new MessageFormatException("Invalid " + DESC
                        + " format: status out of acceptable range: " + status);
            }
    ...
    public static class Game {
        ...
        public Game(String romName, int gameID, String clientType,
                String userName, String players, byte status)
                throws MessageFormatException {
            if (romName.length() == 0) {
                throw new MessageFormatException("Invalid " + DESC
                        + " format: romName.length == 0");
            }
            ...
            if (clientType.length() == 0) {
                throw new MessageFormatException("Invalid " + DESC
                        + " format: clientType.length == 0");
            }

            if (userName.length() == 0) {
                throw new MessageFormatException("Invalid " + DESC
                        + " format: userName.length == 0");
            }
        ...
        public int getLength() {
            ...
            // charset.encode(clientType).remaining() + 1 +
            // charset.encode(userName).remaining() + 1 +
            // charset.encode(players).remaining() + 1 + 1);
            return (romName.length() + 1 + 4 + clientType.length() + 1
                    + userName.length() + 1 + players.length() + 1 + 1);
        }
