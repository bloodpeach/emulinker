...
public class V086Bundle extends ByteBufferMessage {
    ...
    public String toString() {
        ...
        sb.append(DESC + " (" + numToWrite + " messages) (" + getLength()
                + " bytes)");
        ...
        for (int i = 0; i < numToWrite; i++) {
            ...
            sb.append("\tMessage " + (i + 1) + ": " + messages[i].toString()
                    + EmuUtil.LB);
        }
    ...
    public static V086Bundle parse(ByteBuffer buffer) throws ParseException,
            V086BundleFormatException, MessageFormatException {
        return parse(buffer, -1);
    }

    public static V086Bundle parse(ByteBuffer buffer, int lastMessageID)
            throws ParseException, V086BundleFormatException,
            MessageFormatException {
        buffer.order(ByteOrder.LITTLE_ENDIAN);

        if (buffer.limit() < 5) {
            throw new V086BundleFormatException("Invalid buffer length: "
                    + buffer.limit());
        }
        ...
        if (messageCount <= 0 || messageCount > 32) {
            throw new V086BundleFormatException("Invalid message count: "
                    + messageCount);
        }

        if (buffer.limit() < (1 + (messageCount * 6))) {
            throw new V086BundleFormatException("Invalid bundle length: "
                    + buffer.limit());
        }
        ...
        if (msgNum - 1 == lastMessageID) // If added in emux, else default in
                                         // vannila
        {
            messageCount = 1;
            ...
            if (messageLength < 2 || messageLength > buffer.remaining()) {
                throw new ParseException(new StringBuilder()
                        .append("Invalid message length: ")
                        .append(messageLength).toString());
            }

            messages[parsedCount] = V086Message.parse(messageNumber,
            ...
            for (parsedCount = 0; parsedCount < messageCount; parsedCount++) {
                ...
                // throw new
                // V086BundleFormatException("Bundle appears truncated, length = "
                // + buffer.limit());

                int messageNumber = UnsignedUtil.getUnsignedShort(buffer);
                // removed to increase speed
                // if (messageNumber < 0 || messageNumber > 0xFFFF)
                // throw new MessageFormatException("Invalid message number: " +
                // messageNumber);

                if (messageNumber <= lastMessageID) {
                    // buffer.position((buffer.position() + messageLength));
                    // will break here instead of looking for more messages,
                    // should improve speed slightly
                    // there shouldn't be any more valid messages anyway if we
                    // just found the last ID
                    break;
                }
                ...
                if (messageLength < 2 || messageLength > buffer.remaining() /*
                    ...
                    // in
                    // EmuX
                    throw new ParseException("Invalid message length: "
                            + messageLength);
                }

                // messages are purposely added in reverse order here so it's
                // faster to read out in the controller
                // messages.push(V086Message.parse(messageNumber, messageLength,
                // buffer));
                // messages.addFirst(V086Message.parse(messageNumber,
                // messageLength, buffer));
                // messages.add(V086Message.parse(messageNumber, messageLength,
                // buffer));
                messages[parsedCount] = V086Message.parse(messageNumber,
                        messageLength, buffer);
            }
