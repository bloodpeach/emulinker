...
public class ConnectController extends UDPServer {
    ...
    public ConnectController(ThreadPoolExecutor threadPool,
        ...
        if (bufferSize <= 0) {
            throw new ConfigurationException(
                    "controllers.connect.bufferSize must be > 0");
        }
        ...
        for (KailleraServerController controller : controllersArray) {
            ...
            for (int j = 0; j < clientTypes.length; j++) {
                log.debug("Mapping client type " + clientTypes[j] + " to "
                        + controller);
    ...
    protected synchronized void handleReceived(ByteBuffer buffer,
        ...
        // the message set of the ConnectController isn't really complex enough
        // to warrant a complicated request/action class
        ...
        KailleraServerController protocolController = getController(connectMessage
                .getProtocol());
        ...
        if (!accessManager.isAddressAllowed(fromSocketAddress.getAddress())) {
            ...
            try {
                ...
                if (connectedCount > 0) {
                    if (lastAddress.equals(fromSocketAddress.getAddress()
                            .getHostAddress())) {
                        lastAddressCount++;
                        ...
                        if (lastAddressCount >= 4) {
                            lastAddressCount = 0;
                            failedToStartCount++;
                            log.debug("SF MOD: HAMMER PROTECTION (2 Min Ban): "
                                    + EmuUtil
                                            .formatSocketAddress(fromSocketAddress));
                            accessManager.addTempBan(fromSocketAddress
                                    .getAddress().getHostAddress(), 2);
                            return;
                        }
                    }
                    else {
                        lastAddress = fromSocketAddress.getAddress()
                                .getHostAddress();
                        lastAddressCount = 0;
                    }
                }
                ...
                privatePort = protocolController.newConnection(
                        fromSocketAddress, connectMessage.getProtocol());
                ...
                connectedCount++;
                log.debug(protocolController + " allocated port " + privatePort
                        + " to client from "
                        + fromSocketAddress.getAddress().getHostAddress());
                send(new ConnectMessage_HELLOD00D(privatePort),
                        fromSocketAddress);
            }
    ...
    protected void send(ConnectMessage outMessage,
